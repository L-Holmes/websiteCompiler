...
function _showOnlyTopTierFilters(): void {
  ...
  filterTiles.forEach((tile) => {
	const category = _getCategoryOfClickedItem(tile as HTMLElement); //e.g. colours.orange
...
function filterFurtherClicked(element: HTMLElement): void {
	/*
	...
	// ====================
	let childTiers: string[] = _getAllChildren(categoryBeingFilteredFurther) //e.g. ["all.colours.orange.tangerine", "all.colours.orange.tangerine"]
...
--------------------------------------------------
  ...
  console.log(`1`);
    ...
    // 1) 
    const allTableEntries: HTMLElement[] = _get_fresh_filter_tiles(ITEM_WRAPPER_CLASS)
